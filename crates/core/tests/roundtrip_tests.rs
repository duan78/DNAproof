...
use adn_core::codec::goldman_2013::{Goldman2013Decoder, Goldman2013Encoder};
...
fn test_roundtrip_large_file() {
    ...
    assert!(
        sequences.len() > 10,
        "Should have created multiple sequences (with compression)"
    );
}
...
fn test_roundtrip_random_data() {
    ...
    let original: Vec<u8> = (0..500).map(|i| ((i * 17 + 42) % 256) as u8).collect();
